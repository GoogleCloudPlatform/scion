...
// ---------------------------------------------------------------------------
// AllowedGCPProjects normalisation (design §4.1 r7): NewFederationAuthenticator
// lower-cases the Google issuer's AllowedGCPProjects at load time, but must
// NOT touch the unrelated, pre-existing AllowedProjects field.
// ---------------------------------------------------------------------------

func TestFederationAuth_AllowedGCPProjects_NormalisedToLowerCaseAtLoad(t *testing.T) {
	cfg := config.FederationConfig{
		Enabled: true,
		TrustedIssuers: []config.TrustedIssuerConfig{
			{
				...
				IssuerType:         "user",
				AllowedGCPProjects: []string{"My-Project", "OTHER-PROJECT"},
			},
		},
	}
	auth := newTestAuthenticatorWithConfig(t, cfg, "client-id.apps.googleusercontent.com")

	trust, ok := auth.IssuerConfig(googleIssuerHTTPS)
	if !ok {
		t.Fatal("IssuerConfig: issuer not found")
	}
	want := []string{"my-project", "other-project"}
	if len(trust.AllowedGCPProjects) != len(want) {
		t.Fatalf("AllowedGCPProjects = %v, want %v", trust.AllowedGCPProjects, want)
	}
	for i := range want {
		if trust.AllowedGCPProjects[i] != want[i] {
			t.Errorf("AllowedGCPProjects[%d] = %q, want %q", i, trust.AllowedGCPProjects[i], want[i])
		}
	}
}

func TestFederationAuth_AllowedProjects_NotNormalisedToLowerCase(t *testing.T) {
	cfg := config.FederationConfig{
		Enabled: true,
		TrustedIssuers: []config.TrustedIssuerConfig{
			{
				...
				IssuerType:      "hub",
				AllowedProjects: []string{"Proj-Mixed-Case"},
			},
		},
	}
	auth := newTestAuthenticatorWithConfig(t, cfg, "https://hub.example.com")

	trust, ok := auth.IssuerConfig("https://hub.example.com")
	if !ok {
		t.Fatal("IssuerConfig: issuer not found")
	}
	if len(trust.AllowedProjects) != 1 || trust.AllowedProjects[0] != "Proj-Mixed-Case" {
		t.Errorf("AllowedProjects = %v, want untouched [\"Proj-Mixed-Case\"] (design §4.1 r7: only AllowedGCPProjects is normalised)", trust.AllowedProjects)
	}
}
