...
func (s *sqliteWebChatStore) runMigrations() error {
	...
	if err := s.addConversationIDColumn(); err != nil {
		return fmt.Errorf("conversation_id column: %w", err)
	}
	return nil
}
...
// addConversationIDColumn adds the conversation_id column to webchat_topic
// for existing databases that were created before C3 conversation resolution.
func (s *sqliteWebChatStore) addConversationIDColumn() error {
	...
	// ALTER TABLE ADD COLUMN is idempotent in SQLite (ignored if column exists).
	_, err = s.db.Exec(`ALTER TABLE webchat_topic ADD COLUMN conversation_id TEXT`)
	if err != nil {
		// Column may already exist if the table was created with the new schema.
		if !strings.Contains(err.Error(), "duplicate column") {
			return fmt.Errorf("add conversation_id column: %w", err)
		}
	}
