...
// --- 4.3: Harness Config Image Build ---

type harnessConfigBuildRequest struct {
	HarnessConfigID string `json:"harnessConfigId"`
	Tag             string `json:"tag,omitempty"`
}

func (s *Server) handleHarnessConfigImageBuild(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPost {
		MethodNotAllowed(w)
		return
	}
	...
	var req harnessConfigBuildRequest
	if err := readJSON(r, &req); err != nil {
		BadRequest(w, "invalid request body")
		return
	}

	if req.HarnessConfigID == "" {
		ValidationError(w, "harnessConfigId is required", nil)
		return
	}

	hc, err := s.store.GetHarnessConfig(r.Context(), req.HarnessConfigID)
	if err != nil {
		writeError(w, http.StatusNotFound, ErrCodeNotFound, fmt.Sprintf("harness config %q not found", req.HarnessConfigID), nil)
		return
	}
	...
	if !hasDockerfile {
		ValidationError(w, fmt.Sprintf("harness config %q does not contain a Dockerfile", hc.Name), nil)
		return
	}
	...
	go func() {
		...
		log := func(line string) {
			s.events.PublishRaw(subject, imageBuildLogEvent{Type: "log", Line: line})
		}

		tmpDir, err := os.MkdirTemp("", "scion-hc-build-*")
		if err != nil {
			log("error: " + err.Error())
			return
		}
		...
		log(fmt.Sprintf("Materializing %d file(s) from harness-config %q...", len(hc.Files), hc.Name))
		for _, f := range hc.Files {
			objectPath := hc.StoragePath + "/" + f.Path
			reader, _, dlErr := s.storage.Download(ctx, objectPath)
			if dlErr != nil {
				log(fmt.Sprintf("error: failed to download %q: %s", f.Path, dlErr))
				return
			}
			...
			if !strings.HasPrefix(destPath, tmpDir+string(os.PathSeparator)) {
				_ = reader.Close()
				log(fmt.Sprintf("error: invalid file path %q: escapes build directory", f.Path))
				return
			}
			if dir := filepath.Dir(destPath); dir != tmpDir {
				if mkErr := os.MkdirAll(dir, 0o755); mkErr != nil {
					_ = reader.Close()
					log(fmt.Sprintf("error: failed to create directory for %q: %s", f.Path, mkErr))
					return
				}
			}
			...
			if createErr != nil {
				_ = reader.Close()
				log(fmt.Sprintf("error: failed to create file %q: %s", f.Path, createErr))
				return
			}
			...
			_ = outFile.Close()
			if copyErr != nil {
				log(fmt.Sprintf("error: failed to write file %q: %s", f.Path, copyErr))
				return
			}

			if f.Mode != "" {
				mode := os.FileMode(0o644)
				if _, scanErr := fmt.Sscanf(f.Mode, "%o", &mode); scanErr == nil {
		...
		if runtimeBin == "" {
			log("error: no container runtime found (tried docker, podman)")
			return
		}
		...
		log(fmt.Sprintf("Building %s from harness-config %q...", outputImage, hc.Name))
