...
func (_q *UserQuery) QueryOwnedGroups() *GroupQuery {
...
func (_q *UserQuery) QueryMemberships() *GroupMembershipQuery {
...
func (_q *UserQuery) QueryPolicyBindings() *PolicyBindingQuery {
...
// the "owned_groups" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithOwnedGroups(opts ...func(*GroupQuery)) *UserQuery {
	query := (&GroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withOwnedGroups = query
	return _q
}
...
func (_q *UserQuery) WithMemberships(opts ...func(*GroupMembershipQuery)) *UserQuery {
	query := (&GroupMembershipClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithPolicyBindings(opts ...func(*PolicyBindingQuery)) *UserQuery {
	query := (&PolicyBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withOwnedGroups; query != nil {
		if err := _q.loadOwnedGroups(ctx, query, nodes,
			...
			func(n *User, e *Group) { n.Edges.OwnedGroups = append(n.Edges.OwnedGroups, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadOwnedGroups(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	...
	query.Where(predicate.Group(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OwnedGroupsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadMemberships(ctx context.Context, query *GroupMembershipQuery, nodes []*User, init func(*User), assign func(*User, *GroupMembership)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
...
func (_q *UserQuery) loadPolicyBindings(ctx context.Context, query *PolicyBindingQuery, nodes []*User, init func(*User), assign func(*User, *PolicyBinding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(policybinding.FieldUserID)
	}
	query.Where(predicate.PolicyBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PolicyBindingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
