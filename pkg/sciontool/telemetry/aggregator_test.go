...
func TestAggregator_BasicFlow(t *testing.T) {
	...
	// Record model calls
	a.RecordModelEnd(1000, 200, 500)
	a.RecordModelEnd(2000, 300, 1000)

	// Record turns
	a.RecordTurn()
	a.RecordTurn()

	summary := a.Finalize(0, 0, 0, "")
...
func TestAggregator_FinalizeWithSessionEndTokens(t *testing.T) {
	...
	summary := a.Finalize(5000, 1200, 3000, "")
...
func TestAggregator_FinalizeWithError(t *testing.T) {
	...
	summary := a.Finalize(0, 0, 0, "session crashed")
