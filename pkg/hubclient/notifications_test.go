...
func TestNotification_JSON(t *testing.T) {
	t.Run("unmarshal legacy groveId field is not honored", func(t *testing.T) {
		jsonData := `{"id": "n1", "groveId": "p1"}`
		var n Notification
		if err := json.Unmarshal([]byte(jsonData), &n); err != nil {
			t.Fatalf("Unmarshal failed: %v", err)
		}
		if n.ProjectID != "" {
			t.Errorf("ProjectID = %q, want empty (legacy groveId must not be honored)", n.ProjectID)
		}
	})

	t.Run("marshal emits only canonical fields", func(t *testing.T) {
		n := Notification{ID: "n1", ProjectID: "p1"}
...
func TestSubscription_JSON(t *testing.T) {
	t.Run("unmarshal legacy groveId field is not honored", func(t *testing.T) {
		jsonData := `{"id": "s1", "groveId": "p1"}`
		var s Subscription
		if err := json.Unmarshal([]byte(jsonData), &s); err != nil {
			t.Fatalf("Unmarshal failed: %v", err)
		}
		if s.ProjectID != "" {
			t.Errorf("ProjectID = %q, want empty (legacy groveId must not be honored)", s.ProjectID)
		}
	})

	t.Run("marshal emits only canonical fields", func(t *testing.T) {
		s := Subscription{ID: "s1", ProjectID: "p1"}
...
func TestSubscriptionTemplate_JSON(t *testing.T) {
	t.Run("unmarshal legacy groveId field is not honored", func(t *testing.T) {
		...
		if err := json.Unmarshal([]byte(jsonData), &st); err != nil {
			t.Fatalf("Unmarshal failed: %v", err)
		}
		if st.ProjectID != "" {
			t.Errorf("ProjectID = %q, want empty (legacy groveId must not be honored)", st.ProjectID)
		}
	})

	t.Run("marshal emits only canonical fields", func(t *testing.T) {
		st := SubscriptionTemplate{ID: "st1", ProjectID: "p1"}
