...
// Usage:
//
//	a2a-testserver -port=PORT -database-url=URL [-project=PROJ] [-agent=AGENT]
//
// The server prints "READY <pid>" to stdout once listening, then serves
...
func main() {
	...
	// Wrap with middleware that sets RouteInfo in context.
	...
	server := &http.Server{Handler: routeHandler}
