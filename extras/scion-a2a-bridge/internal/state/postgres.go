...
func (s *PostgresStore) LatestTaskEventID(ctx context.Context, taskID string) (int64, error) {
	var maxID int64
	err := s.db.QueryRowContext(ctx,
		`SELECT COALESCE(MAX(id), 0) FROM a2a_task_events WHERE task_id = $1`, taskID,
	).Scan(&maxID)
	if err != nil {
