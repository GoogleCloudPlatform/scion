...
`hubBearer` (Google credential pass-through) and `geGoogle` (Google credential exchange, **deprecated** — superseded by `hubBearer`, which forwards the same credential without an exchange round trip) are not offered in the **Auth scheme** dropdown. Configure either one via the `auth.scheme` key in `scion-a2a-bridge.yaml` instead — see `scion-a2a-bridge.yaml.sample` for the `hubBearer` block. `geGoogle` is not removed and continues to work; new deployments should configure `hubBearer` instead.
