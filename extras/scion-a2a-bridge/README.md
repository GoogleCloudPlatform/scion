...
`hubBearer`/`geGoogle` set in YAML can't be overridden from the admin UI: saving A2A integration settings pushes the dropdown's `auth_scheme` value, but a scheme not offered in that dropdown is pinned and the push is ignored. The bridge logs a `WARN` naming both the YAML and the pushed scheme whenever this happens. (Bridges built before this protection was added should be upgraded; on those, the admin UI has no way to remove a previously pushed `auth_scheme` on its own.)
