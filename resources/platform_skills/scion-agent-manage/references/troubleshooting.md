...
| `scion start` fails with 422 `no_runtime_broker` | Broker reconnected but internal status has not fully reconciled yet | Wait 30–60s and retry. If persistent, escalate — the hub process may need a restart. Reduce concurrent starts if it recurs under load |
