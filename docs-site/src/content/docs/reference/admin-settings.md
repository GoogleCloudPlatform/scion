...
2. **Agent Defaults Card** (with dedicated sub-tabs):
   ...
   - Adds a hub-level **Default GCP Identity Mode** (`agent_defaults.default_gcp_identity_mode`) with an optional service account picker (`default_gcp_identity_service_account_id`). It is the fallback for new agents when neither the create request nor the project's default GCP identity sets a mode. **Passthrough** applies only on the Hub's embedded broker, and agents on any other broker get **Block**. **Assign** accepts only a verified hub-scoped service account and requires `gcp_iam_check_mode: enforce`. Other values are rejected with `422`. The setting works in both database and file (SQLite) mode. See [Hub-Default GCP Identity](/scion/hosted/ha/permissions/#hub-default-gcp-identity).
