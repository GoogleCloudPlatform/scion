...
[Google Cloud Agent Registry](https://cloud.google.com/products/agent-registry) (preview).
...
- A GCP service account with the `agentregistry.editor` role on the
  target GCP project, registered as a managed SA in the Scion Hub.
- The A2A Bridge deployed and accessible at an external URL.
- `PROJECT_SLUG` and `AGENT_SLUG` variables are automatically available
  as trusted lifecycle-hook variables.
